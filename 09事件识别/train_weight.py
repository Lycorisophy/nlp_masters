...
if __name__ == "__main__" :
    ...
    setting = []
    for i in range(100):
        weight = [random.random(), random.random(), random.random()]  # 权重参数，由句子中各要素长度和初始权重决定，分别表示一般名词，句子和动词的权重
        data_path = 'data'
        all_filenames = glob.glob(data_path + '/*.txt')
        n_right = 0
        for filename in all_filenames:
            ...
            for line in readlines(filename):
                ...
                if preguess[0][1] == categories:
                    n_right += 1
        setting.append([n_right, weight[0], weight[1], weight[2]])
    max = 0
